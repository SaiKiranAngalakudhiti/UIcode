...
function DataTable({ tableData, clothCategory, fetchRecord }) {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://52.20.201.221:4000/clothAPI/addClothing", {
      method: "POST",
      body: formData,
      headers: {
  ...
  const confirmDelete = () => {
    fetch(`http://52.20.201.221:4000/clothAPI/deleteClothing/${itemId}`, {
      method: "DELETE",
      headers: {
  ...
  const handleUpdate = async (event) => {
    ...
    await fetch("http://52.20.201.221:4000/clothAPI/editClothing", {
      method: "PUT",
      body: formData,
      headers: {
            ...
            <FormControl fullWidth sx={{ marginBottom: "10px" }}>
              ...
              >
                {clothCategory?.map((item, index) => {
                  return (
                    <MenuItem value={item?.category_name} key={index}>
                      {item?.category_name}
                    </MenuItem>
                  );
                })}
      ...
      >
        ...
        <Grid container spacing={2} px="20px">
          ...
          {!file && values.image && <Typography>
            {values.image?.replace("http://52.20.201.221:4000/images/","")}
            </Typography>}
          </Grid>
        </Grid>
