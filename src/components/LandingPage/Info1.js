...
const Info1 = ({weekly_data,updateCount,timezone_id,set_hourly_info,loc_id,setSelectedHourlyButton,set_time_selected, set_pref_id,
    ...
    //const showArrowsWeekly = scrollRefWeekly.current && scrollRefWeekly.current.scrollWidth > scrollRefWeekly.current.clientWidth;
    //console.log("inf"+scrollRefWeekly?.current?.scrollWidth)
    //console.log("inf"+scrollRefWeekly?.current?.clientWidth)
    //console.log("Weeklydata"+scrollRefWeekly.current && scrollRefWeekly.current.scrollWidth > scrollRefWeekly.current.clientWidth)
    ...
    const getHourlyData= async(locid,dateWeekly,day)=>{
        try {
            ...
            let selectedTime;
            if(day === 'Today'){
                res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherDataHourly?id="+locid+"&tz="+timezone_id+"&date="+dateWeekly+"&day=today");
            }
            else{
                res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherDataHourly?id="+locid+"&tz="+timezone_id+"&date="+dateWeekly+"&day=other");
            }
