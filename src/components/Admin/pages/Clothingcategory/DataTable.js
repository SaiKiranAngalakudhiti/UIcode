...
function DataTable({ tableData, fetchRecord }) {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://52.20.201.221:4000/clothCatAPI/addClothingCategory", {
      method: "POST",
      body: JSON.stringify({
  ...
  const confirmDelete = () => {
    fetch(
      `http://52.20.201.221:4000/clothCatAPI/deleteClothingCategory/${itemId}`,
      {
        method: "DELETE",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
  ...
  const handleUpdate = async (event) => {
    ...
    await fetch("http://52.20.201.221:4000/clothCatAPI/editClothCategory", {
      method: "PUT",
      body: JSON.stringify({
