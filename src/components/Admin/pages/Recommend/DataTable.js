...
function DataTable({
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://52.20.201.221:4000/recommendAPI/addRecommend", {
      method: "POST",
      body: JSON.stringify({
  ...
  const confirmDelete = () => {
    fetch(`http://52.20.201.221:4000/recommendAPI/deleteRecommend/${itemId}`, {
      method: "DELETE",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        "x-access-token": localStorage.getItem("jwtToken"),
  ...
  const handleUpdate = async (event) => {
    ...
    await fetch("http://52.20.201.221:4000/recommendAPI/editRecommend", {
      method: "PUT",
      body: JSON.stringify({
...
if(values.clothing_category_id){
  ...
  fetch(`http://52.20.201.221:4000/clothAPI/getClothingByCategory?id=${values.clothing_category_id}`)
  .then((res) => res.json())
  .then((data) => {
