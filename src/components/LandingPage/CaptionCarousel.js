...
const CaptionCarousel = ({setDay,set_hourly_info,dataHourlyInSearch,loc_id,timezone_id,recommend_data,set_pref_id, pref_id,
  ...
  const getHourlyData= async()=>{
    ...
    try {
      let res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherDataHourly?id="+loc_id+"&tz="+timezone_id+"&date=2023-04-10&day=today");
  ...
  return (
      <div>
        ...
        <Modal isOpen={isOpen} onRequestClose={toggleModal} contentLabel="My dialog" className="mymodal" id="feedback">
            <Button position={"absolute"} right={"5px"} top={"5px"} alignContent={"center"} onClick={toggleModal} padding={"5px"}>
                <CloseIcon />
            </Button>
        ...
        <div className='cont' id="cont">
          <div>
            <Text pl={"2vh"} fontSize={"xl"} fontFamily={"Karma"} float={"left"} textAlign="center">
              {recommDay + "'s ("+timeSelected+" "+timezone_id_abbr+") clothing suggestions for " + style.feel + " and " + pref + " preference"}
            </Text>
            {recommDay !== 'Today' &&
