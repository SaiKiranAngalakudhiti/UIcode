...
function DataTable({ tableData }) {
  ...
  const handleSubmit = async () => {
    ...
    await fetch("http://52.20.201.221:4000/prefAPI/addPreference", {
      method: "POST",
      body: JSON.stringify({
  ...
  const result=  await fetch(`http://52.20.201.221:4000/prefAPI/deletePreference/${itemId}`, {
      method: "DELETE",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        "x-access-token": localStorage.getItem("jwtToken"),
  ...
  const handleUpdate = async (event) => {
    ...
    await fetch("http://52.20.201.221:4000/prefAPI/editPreference", {
      method: "PUT",
      body: JSON.stringify({
