...
const [previousData, setPreviousData] = useState(false);
  ...
  async function getpreferences() {
    try {
        let res = await axios.get("http://52.20.201.221:4000/prefAPI/getPreferenceSport");
  ...
  //   try {
  //     let res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherData/" + searchip);
...
const defaultCity = () => {
    if (navigator.geolocation) 
    {
      ...
      async function getCity() {
          try {
              const url = "http://52.20.201.221:4000/locationAPI/defaultCity/" + lat + "/" + long;
      ...
      if(resDisabled.data.message == "city does not exist"){
        fetch(`https://community-zippopotamus.p.rapidapi.com/us/${data.postalCaode}`, {
          ...
          .then(async(response) =>   {
            ...
            let checkAgain = await axios.get("http://52.20.201.221:4000/locationAPI/checkLocationPostal?postal="+data.postalCaode);
  ...
  return (
    <ChakraProvider>
      ...
      <div className='Landing'>
        ...
        postalCodeData = {postalCodeData} cityData={cityData}
        ...
        previousData = {previousData} defaultPrefIndex={defaultPrefIndex}/>
