...
function DataTable({ tableData, fetchRecord }) {
  ...
  const handleSubmit = async () => {
    if (data.some((x) => x.postalCaode == values.postalCaode)) {
      setOpenAlert(true);
      setToastMessge("Postal Code already exists!");
      return;
    }
    if (data.some((x) => x.city === values.city)) {
      setOpenAlert(true);
      setToastMessge("City name already exists!");
      return;
    }
    if(!byZipCode){
      if (values.postalCaode === "" || !(zipCodeData && zipCodeData[0] && zipCodeData[0].latitude)) {
        setOpenAlert(true);
        setToastMessge("Invalid postal code!");
        return;
      }
    }
    else{
 if (values.postalCaode === "" || values.latitude === "") {
    ...
    await fetch("http://52.20.201.221:4000/locationAPI/addLocation", {
      method: "POST",
      body: JSON.stringify(
  ...
  const confirmDelete = () => {
    fetch(`http://52.20.201.221:4000/locationAPI/deleteLocation/${itemId}`, {
      method: "DELETE",
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        "x-access-token": localStorage.getItem("jwtToken"),
  ...
  const confirmDeleteBulk = async () => {
    try {
      const result = await fetch(
        "http://52.20.201.221:4000/locationAPI/deleteLocationBulk",
        {
          method: "DELETE",
          body: JSON.stringify({
  ...
  const confirmUpdateBulk = async () => {
    try {
      const result = await fetch(
        "http://52.20.201.221:4000/locationAPI/editLocationBulk",
        {
          method: "PUT",
          body: JSON.stringify({
            location_ids: selectedRows,
