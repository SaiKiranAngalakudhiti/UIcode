...
const Search = ({props,setStyle,set_pref,set_timezone,set_hourly_info,set_weekly_data,cityData,cityParent, defaultPrefIndex,
    setSelectedHourlyButton,setSelectedWeeklyButton,set_time_selected,style,postal_code,postalCodeData,set_timezone_abbr,isLocationEnable,
    set_location_id,pref,dataHourlyInSearch,dsearch,set_recommend_info,setPreviousData, previousData,setlatslongs, pref_id, set_pref_id}) => {
    let timeZoneId='';
    const nextDates = [];
  ...
  // Callback function to receive value from child component
    ...
    const checkifDataExists=async(valuedData)=>{
        ...
        try {
            ...
            if(valuedData.postalCaode==="")
                res = await axios.get("http://52.20.201.221:4000/locationAPI/checkLocation?city="+valuedData.city);
            else
                res = await axios.get("http://52.20.201.221:4000/locationAPI/checkLocationPostal?postal="+valuedData.postalCaode);
    ...
    const getHourlyData= async(locid)=>{
        ...
        if(locid!=0){
            try {
            let res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherDataHourly?id="+locid+"&tz="+timeZoneId+"&date=2023-04-10&day=today");
      ...
      const getWeeklyData = async(locid) => {
        ...
        if(locid != 0){
          ...
          for (let i = 0; i <= 5; i++) {
            ...
            try {
              let res = [];
              res = await axios.get("http://52.20.201.221:4000/weatherAPI/getWeatherDataWeekly?id="+locid+"&tz="+timeZoneId+"&date="+formattedDate);
    ...
    const getrecommends = async(appTemp,prefid,feelid) => {
        set_recommend_info([]);
        try {
            let res = await axios.get("http://52.20.201.221:4000/recommendAPI/getRecommendations?apparentTemp="+appTemp+"&sport_id="+prefid+"&feel_id="+feelid);
            ...
            if(data.postalCaode == ""){
                if(data.country !== '' && data.state !== '' && data.city !== ''){
                    fetch(`https://community-zippopotamus.p.rapidapi.com/${data.country}/${data.state}/${data.city}`, {
                        ...
                        fetch("http://52.20.201.221:4000/locationAPI/addLocation", {
                            method: "POST",
                        ...
                        .then(async(response) => {
                            ...
                            res2 = await axios.get("http://52.20.201.221:4000/locationAPI/checkLocationPostal?postal="+dataByPostalCode.postal_code);
                            if(res2.data.message == "city does not exist"){   
                            }
                            else{
                                res3 = await axios.get("http://52.20.201.221:4000/locationAPI/addNewData/"+res2.data.location_id);
                ...
                let resDisabled = await axios.get("http://52.20.201.221:4000/locationAPI/checkLocationPostalDisabled?postal="+data.postalCaode);
                ...
                await fetch("http://52.20.201.221:4000/locationAPI/editLocation", {
                method: "PUT",
                body: JSON.stringify({
                ...
                if(response.ok){
                    newresponse = await axios.get("http://52.20.201.221:4000/locationAPI/addNewData/"+resDisabled.data.location_id);
    ...
    return (
        ...
        <div className='searchdiv'>
            ...
            <Settings position="absolute" top="0" right="0" style={style} setStyle={setStyle} pref={pref} defaultPrefIndex={defaultPrefIndex}
            ...
            {postal_code!=='' &&	
                <Text textAlign={"center"} fontSize="3xl" color={"white"} mt={"1vh"} fontFamily={"Karma"} >{cityData}({postal_code})</Text>	
            }	
            {	
